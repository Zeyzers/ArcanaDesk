    ...
    .forEach((entry) => {
      ...
      const rollsText = entry.breakdown
        .map((p) => {
          ...
          const keptStr = p.kept.length
            ? `<span class="kept">${p.kept.join(",")}</span>`
            : "";
...
const renderTurns = () => {
  ...
  state.turnOrder.forEach((entry) => {
    ...
    li.querySelector('[data-action="activate"]').addEventListener(
      "click",
      () => {
...
nextTurn.addEventListener("click", () => {
  ...
  // decrement durations and drop expired
...
const getCurrentNote = () =>
  state.notes.find((n) => n.id === state.currentNoteId);
